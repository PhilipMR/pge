...
namespace ImGuiJako
{
    ...
    bool
    equals(InputIt1 first1, InputIt1 last1, InputIt2 first2, InputIt2 last2, BinaryPredicate p)
    {
    ...
    void
    TextEditor::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
    {
    ...
    void
    TextEditor::SetPalette(const Palette& aValue)
    {
        mPaletteBase = aValue;
    }

    std::string
    TextEditor::GetText(const Coordinates& aStart, const Coordinates& aEnd) const
    {
        std::string result;
    ...
    TextEditor::GetActualCursorCoordinates() const
    {
    ...
    static int
    UTF8CharLength(TextEditor::Char c)
    {
        if ((c & 0xFE) == 0xFC)
            return 6;
        if ((c & 0xFC) == 0xF8)
    ...
    static inline int
    ImTextCharToUtf8(char* buf, int buf_size, unsigned int c)
    {
        if (c < 0x80) {
            buf[0] = (char)c;
            return 1;
    ...
    void
    TextEditor::Advance(Coordinates& aCoordinates) const
    {
    ...
    void
    TextEditor::DeleteRange(const Coordinates& aStart, const Coordinates& aEnd)
    {
        assert(aEnd >= aStart);
        assert(!mReadOnly);
    ...
    int
    TextEditor::InsertTextAt(Coordinates& /* inout */ aWhere, const char* aValue)
    {
        assert(!mReadOnly);
    ...
    void
    TextEditor::AddUndo(UndoRecord& aValue)
    {
        assert(!mReadOnly);
    ...
    int
    TextEditor::GetCharacterIndex(const Coordinates& aCoordinates) const
    {
    ...
    int
    TextEditor::GetCharacterColumn(int aLine, int aIndex) const
    {
    ...
    int
    TextEditor::GetLineCharacterCount(int aLine) const
    {
        if (aLine >= mLines.size())
            return 0;
        auto& line = mLines[aLine];
    ...
    int
    TextEditor::GetLineMaxColumn(int aLine) const
    {
        if (aLine >= mLines.size())
            return 0;
        auto& line = mLines[aLine];
    ...
    bool
    TextEditor::IsOnWordBoundary(const Coordinates& aAt) const
    {
    ...
    void
    TextEditor::RemoveLine(int aStart, int aEnd)
    {
        assert(!mReadOnly);
        assert(aEnd >= aStart);
    ...
    void
    TextEditor::RemoveLine(int aIndex)
    {
        assert(!mReadOnly);
    ...
    TextEditor::Line&
    TextEditor::InsertLine(int aIndex)
    {
        assert(!mReadOnly);
    ...
    std::string
    TextEditor::GetWordUnderCursor() const
    {
    ...
    std::string
    TextEditor::GetWordAt(const Coordinates& aCoords) const
    {
    ...
    ImU32
    TextEditor::GetGlyphColor(const Glyph& aGlyph) const
    {
        if (!mColorizerEnabled)
    ...
    void
    TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
    {
        mWithinRender          = true;
        mTextChanged           = false;
        mCursorPositionChanged = false;
    ...
    void
    TextEditor::SetText(const std::string& aText)
    {
        mLines.clear();
    ...
    void
    TextEditor::SetTextLines(const std::vector<std::string>& aLines)
    {
        mLines.clear();
    ...
    void
    TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
    {
        assert(!mReadOnly);
    ...
    void
    TextEditor::SetReadOnly(bool aValue)
    {
        mReadOnly = aValue;
    }

    void
    TextEditor::SetColorizerEnable(bool aValue)
    {
        mColorizerEnabled = aValue;
    }

    void
    TextEditor::SetCursorPosition(const Coordinates& aPosition)
    {
    ...
    void
    TextEditor::SetSelectionStart(const Coordinates& aPosition)
    {
    ...
    void
    TextEditor::SetSelectionEnd(const Coordinates& aPosition)
    {
    ...
    void
    TextEditor::SetSelection(const Coordinates& aStart, const Coordinates& aEnd, SelectionMode aMode)
    {
    ...
    void
    TextEditor::SetTabSize(int aValue)
    {
        mTabSize = std::max(0, std::min(32, aValue));
    }

    void
    TextEditor::InsertText(const std::string& aValue)
    {
        InsertText(aValue.c_str());
    }

    void
    TextEditor::InsertText(const char* aValue)
    {
        if (aValue == nullptr)
            return;
    ...
    void
    TextEditor::MoveUp(int aAmount, bool aSelect)
    {
    ...
    void
    TextEditor::MoveDown(int aAmount, bool aSelect)
    {
    ...
    static bool
    IsUTFSequence(char c)
    {
        return (c & 0xC0) == 0x80;
    }

    void
    TextEditor::MoveLeft(int aAmount, bool aSelect, bool aWordMode)
    {
        if (mLines.empty())
            return;
    ...
    void
    TextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void
    TextEditor::MoveTop(bool aSelect)
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void
    TextEditor::TextEditor::MoveBottom(bool aSelect)
    {
    ...
    void
    TextEditor::MoveHome(bool aSelect)
    {
        auto oldPos = mState.mCursorPosition;
    ...
    void
    TextEditor::MoveEnd(bool aSelect)
    {
        auto oldPos = mState.mCursorPosition;
    ...
    bool
    TextEditor::CanRedo() const
    {
        return !mReadOnly && mUndoIndex < (int)mUndoBuffer.size();
    }

    void
    TextEditor::Undo(int aSteps)
    {
    ...
    void
    TextEditor::Redo(int aSteps)
    {
        while (CanRedo() && aSteps-- > 0)
            mUndoBuffer[mUndoIndex++].Redo(this);
    }

    const TextEditor::Palette&
    ...
    std::string
    TextEditor::GetSelectedText() const
    {
        return GetText(mState.mSelectionStart, mState.mSelectionEnd);
    }

    std::string
    TextEditor::GetCurrentLineText() const
    {
    ...
    void
    TextEditor::ProcessInputs()
    {}

    void
    TextEditor::Colorize(int aFromLine, int aLines)
    {
    ...
    void
    TextEditor::ColorizeRange(int aFromLine, int aToLine)
    {
    ...
    float
    TextEditor::TextDistanceToLineStart(const Coordinates& aFrom) const
    {
    ...
    void
    TextEditor::UndoRecord::Undo(TextEditor* aEditor)
    {
        if (!mAdded.empty()) {
    ...
    void
    TextEditor::UndoRecord::Redo(TextEditor* aEditor)
    {
        if (!mRemoved.empty()) {
        ...
        if (!inited) {
            ...
            static const char* const identifiers[]
                = {"abort",   "abs",           "acos",    "asin",          "atan",     "atexit", "atof",    "atoi",    "atol",   "ceil",    "clock",
                   "cosh",    "ctime",         "div",     "exit",          "fabs",     "floor",  "fmod",    "getchar", "getenv", "isalnum", "isalpha",
                   "isdigit", "isgraph",       "ispunct", "isspace",       "isupper",  "kbhit",  "log10",   "log2",    "log",    "memcmp",  "modf",
                   "pow",     "printf",        "sprintf", "snprintf",      "putchar",  "putenv", "puts",    "rand",    "remove", "rename",  "sinh",
                   "sqrt",    "srand",         "strcat",  "strcmp",        "strerror", "time",   "tolower", "toupper", "std",    "string",  "vector",
                   "map",     "unordered_map", "set",     "unordered_set", "min",      "max"};
